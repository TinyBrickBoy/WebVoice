...
type State = Pick<PlayerState, "uniqueId" | "name" | "textureHash" | "speaking" | "muted" | "deafened" | "register">
            ...
            {!hideHead &&
                <SpeakingOutline state={state} className={`rounded-md`}>
                    <Tooltip align={"top"} hint={<MinecraftComponent component={name}/>}>
                        <CraftHead className={"rounded-md w-12"} textureHash={textureHash} uuid={uniqueId}/>
                    </Tooltip>
                </SpeakingOutline>}
